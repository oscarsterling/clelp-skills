...
class TestHeuristics:
    ...
    def test_opus_model_boost(self):
        job = make_job(payload_text="do something", model="opus")
        result = classify_job(job)
...
class TestRealJobs:
    ...
    def test_metrics_compiler_is_mechanical(self, sample_jobs):
        job = find_job(sample_jobs, "Metrics Snapshot Compiler")
        result = classify_job(job)
        assert result.classification == Classification.MECHANICAL

    def test_dependency_update_check_is_mechanical(self, sample_jobs):
        job = find_job(sample_jobs, "Dependency Update Check")
        result = classify_job(job)
        assert result.classification == Classification.MECHANICAL
    ...
    def test_market_research_digest_is_reasoning(self, sample_jobs):
        job = find_job(sample_jobs, "Market Research Digest")
        result = classify_job(job)
        assert result.classification == Classification.REASONING

    def test_daily_ops_brief_is_reasoning(self, sample_jobs):
        job = find_job(sample_jobs, "Daily Ops Brief")
        result = classify_job(job)
    ...
    def test_full_payload_daily_brief_is_reasoning(self):
        ...
        job = make_job(
            ...
            payload_text="Generate the daily ops brief. Include overnight alerts, top priorities and reminders, morning highlights with business opportunities, and curated insights. Write up trending opportunities. Summarize and compose the content for the team.",
            model="sonnet",
        )
        result = classify_job(job)
        assert result.classification == Classification.REASONING
    ...
    def test_full_payload_eod_summary_is_reasoning(self):
        ...
        job = make_job(
            name="eod-summary",
            payload_text="Generate the end-of-day summary. Read today's status file. Summarize what got done, what's in progress, what's pending. Write a brief with bullet points covering the real highlights. Synthesize and compose the daily output.",
            model="sonnet",
        )
        result = classify_job(job)
        assert result.classification == Classification.REASONING
...
class TestPrintSummary:
    ...
    def test_contains_job_names(self, sample_jobs):
        ...
        assert "Health Check" in summary or "Backup" in summary or "Research" in summary
